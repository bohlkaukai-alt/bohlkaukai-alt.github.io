...
function updateHeader(page) {
    ...
    if (back) {
        c.innerHTML = `<div class="app-header"><button class="back-arrow" onclick="goBack()" aria-label="Zurück">←</button><span class="header-title">Zurück</span>${toggle}</div>`;
    } else {
        c.innerHTML = `<div class="app-header"><h1><span>💼 MiniJob</span> Finder</h1>${toggle}</div>`;
    }
