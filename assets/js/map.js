...
async function initMap() {
    ...
    snap.docs.forEach(d => {
        ...
        const popup = `<strong>${escapeHtml(j.title)}</strong><br>${escapeHtml(j.category)}<br>${escapeHtml(j.location)}<br>${formatPayment(j.payment)}<br><button onclick="navigateTo('job-detail','${d.id}')">Details</button>`;
